/**
  ...
  * @date           : 8 April 2023
  * @author		    : NR
  * @brief          : Include file for the driver of the RDA 5807M chip (FM radio via I2c)
  *
  ...
  *
  * @status : In course (OK for Step 1 to 5)
  *
  ******************************************************************************
  */
...
//#include <stm32f10x_i2c.h>	// changed by NR
#include "stm32f2xx_hal.h"		// NR
#include "i2c.h"				// NR
#include "ssd1306.h"			// NR : To display the current frequency in seek search mode (show_freq() )
typedef enum
{
    FALSE,
    TRUE,
...
//NR : return value for the DRA driver
typedef enum
{
	RDA_OK,
	RDA_Init_Err1,
...
#define I2C_ADDR_DIRECT_ACCESS  (0x11<< 1)    //!< Can be used to access a given register at a time.
#define I2C_ADDR_FULL_ACCESS    (0x10<< 1)    //!< Can be used to access a set of register at a time.
...
// take care : seek mode is managed also through register 5
#define RDA_SEEK_WRAP  0     //!< SEEK MODE : Wrap at the upper or lower band limit and continue seeking
#define RDA_SEEK_STOP  1     //!< SEEK_MODE : Stop seeking at the upper or lower band limit
#define RDA_SEEK_DOWN  0     //!< SEEKUP    : Seek Up
#define RDA_SEEK_UP    1     //!< SEEKUP    : Seek Down

// NR : To check correspond to nothing clear
...
#define RDA_FM_BAND_USA_EU       0  //!< BAND 0 : 87.5–108 MHz (US / Europe, Default)
#define RDA_FM_BAND_JAPAN_WIDE   1  //!< BAND 1 : 76–91 MHz (Japan wide band)
#define RDA_FM_BAND_WORLD        2  //!< BAND 2 : 76–108 MHz (world wide)
#define RDA_FM_BAND_SPECIAL      3  //!< BAND 3 : 65 –76 MHz(East Europe) or 50 - 65MHz(see bit 9 of register 0x06)

//------------------------------------------------------------------
// Register 05
//------------------------------------------------------------------
#define RDA_SEEK_MODE_2bits_DEFAULT 0
#define RDA_SEEK_MODE_2bits_RSSI    2
 ...
 * @details Clock type configuration; seek operation; Stereo/Mono; Bass;  and Audio configuration
...
typedef union {
    struct
    {
        uint8_t ENABLE : 1; //LSB   //!< Power Up Enable; 0 = Disabled; 1 = Enabled
        ...
        uint8_t DHIZ : 1;   //MSB   //!< Audio Output High-Z Disable; 0 = High impedance; 1 = Normal operation
    } refined;
    uint16_t raw;
} RDA_Reg02;
...
// NR copy from the RDA_5807.c <== to remove after the debug
typedef struct
{
    // REG01
	RDA_Reg01 reg01;
...
// NR : End part to remove after debug

//NR : for debug : normally an internal API function
 ...
 * @param  showfreq() : NULL or the function displaying a slider or the current test frequency
...
// --- Variables et fonctions RDS pour le nom de la station ---
extern char rda_station_name[9]; // 8 caractères max pour le nom RDS + 1 caractère de fin de chaîne
